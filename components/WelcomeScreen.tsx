...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onStart }) => {
  return (
    <div className="text-center flex flex-col items-center p-4 animate-fade-in">
      ...
      <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-2">Welcome to NutriAura AI</h2>
      <p className="text-slate-600 dark:text-slate-400 mb-8 max-w-md">
        Your body's intelligent wellness mirror. Get personalized insights by analyzing your face and lifestyle.
      </p>
      <button
        onClick={onStart}
      ...
      <p className="text-xs text-slate-400 dark:text-slate-500 mt-6 max-w-sm">
        Disclaimer: NutriAura AI provides wellness suggestions and is not a substitute for professional medical advice, diagnosis, or treatment.
      </p>
    </div>
  );
};
